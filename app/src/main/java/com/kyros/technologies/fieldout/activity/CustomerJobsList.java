...
import com.kyros.technologies.fieldout.adapters.CustomerJobsListAdapter;
...
/**
 * Created by Rohin on 08-02-2018.
 */

public class CustomerJobsList extends AppCompatActivity {

    private PreferenceManager store;
    ArrayList<CommonJobs> commonJobsArrayList = new ArrayList<CommonJobs>();
    private RecyclerView jobs_month_recycler;
    private String customerjobs=null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            customerjobs=bundle.getString("customerjobs");
        }catch (Exception e){
            e.printStackTrace();
        }

        try {
            JSONArray techallJobs=new JSONArray(customerjobs);
            for (int a=0;a<techallJobs.length();a++){
                JSONObject second = techallJobs.getJSONObject(a);
                JSONArray jobs=second.getJSONArray("jobs");
                for (int i=0;i<jobs.length();i++){
                    JSONObject third = jobs.getJSONObject(a);
            ...
            if (commonJobsArrayList.size()!=0){
                ...
                CustomerJobsListAdapter customerJobsListAdapter = new CustomerJobsListAdapter(CustomerJobsList.this, commonJobsArrayList);
                jobs_month_recycler.setAdapter(customerJobsListAdapter);
                customerJobsListAdapter.notifyDataSetChanged();
            }
        }catch (Exception e){
            e.printStackTrace();
        }
